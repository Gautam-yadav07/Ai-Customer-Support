...
const systemPrompt = "You are an AI support assistant.";
...
export async function POST(req) {
    ...
    const rawResponseText = await response.text();
    console.log("Raw Response Text:", rawResponseText); // Debugging output

    try {
        const routerData = JSON.parse(rawResponseText);
        console.log("Parsed Response:", routerData);

        if (routerData.choices && routerData.choices.length > 0) {
            const messageContent = routerData.choices[0].message?.content;
            return NextResponse.json({ message: messageContent }, { status: 200 });
        } else {
            console.error("Invalid format for choices:", routerData);
            return NextResponse.json({ error: "Invalid response format", details: routerData }, { status: 500 });
        }
    } catch (error) {
        console.error("Error parsing response:", error, rawResponseText);
        return NextResponse.json({ error: "Error parsing response", details: rawResponseText }, { status: 500 });
    }
}
